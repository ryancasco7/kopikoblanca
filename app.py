...
from session_manager import validate_session, delete_session, cleanup_expired_sessions
...
def main():
    ...
    try:
        ...
        # FIRST: Check for session restoration from query params (highest priority)
        # This handles page refreshes where JavaScript has already added the token to URL
        try:
            query_params = st.query_params
            if 'restore_session' in query_params and not st.session_state.logged_in:
                session_token = query_params['restore_session']
                ...
                if session_token:
                    ...
                    if session_data:
                        ...
                        st.session_state.session_restore_attempted = False  # Reset flag
                        logger.info(f"Session restored from query params for: {session_data['username']}")
                        # Clear query params and rerun immediately
                        st.query_params.clear()
                        st.rerun()
        except Exception as e:
            logger.error(f"Error in early session restoration: {e}")
        
        # SECOND: If not logged in and no query param, inject JavaScript to check browser storage
        # This must happen BEFORE showing auth page - use markdown for earlier execution
        if not st.session_state.logged_in and 'restore_session' not in st.query_params:
            # Check if we've already tried to restore (prevent infinite loops)
            if 'session_restore_attempted' not in st.session_state:
                st.session_state.session_restore_attempted = True
                
                # Inject JavaScript using markdown for earlier execution
                # Use both immediate execution and DOMContentLoaded as fallback
                st.markdown("""
                    <script>
                    // Immediate execution
                    (function() {
                        if (!window.location.search.includes('restore_session')) {
                            let sessionToken = sessionStorage.getItem('session_token') || 
                                             ...
                                             (document.cookie.match(/session_token=([^;]+)/) ? document.cookie.match(/session_token=([^;]+)/)[1] : null);
        ...
        # Try to restore session from token (for page refresh persistence)
        if not st.session_state.logged_in and st.session_state.session_token:
            session_data = validate_session(st.session_state.session_token)
            if session_data:
                # Restore session
                st.session_state.logged_in = True
                st.session_state.username = session_data['username']
                st.session_state.user_role = session_data['role']
                logger.info(f"Session restored for user: {session_data['username']}")
            else:
        ...
        # Show auth page if not logged in (after all restoration attempts)
        ...
        with col2:
            ...
            if st.button("🚪 Logout", type="secondary", use_container_width=True):
                ...
                # Clear browser storage (all locations)
                st.markdown("""
                    <script>
                    (function() {
