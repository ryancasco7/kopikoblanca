    ...
    try:
        ...
        if save_users(users):
            ...
            log_activity(
                ...
                user=username,
                description=f"New teacher account created: {username}",
                details={'role': ROLE_TEACHER}
            )
...
def login(username: str, password: str) -> Tuple[bool, str, Optional[str], Optional[str]]:
    """
    ...
    if not verify_password(password, user.get('password', '')):
        ...
        if user['failed_attempts'] >= 5:
            return False, "Account locked due to too many failed attempts. Please contact administrator.", None, None
        
        return False, f"Invalid username or password ({user['failed_attempts']}/5 attempts)", None, None
    ...
    session_token = create_session(matching_username, role)
    
    # Log login activity
    log_activity(
        activity_type='login',
        user=matching_username,
        description=f"User logged in successfully",
        details={'role': role, 'last_login': user['last_login']}
    )
    
    return True, "Login successful!", role, session_token
        ...
        with st.form("login_form"):
            ...
            if submit:
                if username and password:
                    success, message, role, session_token = login(username, password)
                    if success:
                        st.session_state.logged_in = True
                        st.session_state.username = username
                        st.session_state.user_role = role
                        st.session_state.session_token = session_token
                        st.session_state.session_restore_attempted = False  # Reset flag on successful login
                        
                        # Store session token in browser sessionStorage and cookies for persistence
                        # Use markdown for more reliable execution
                        st.markdown(f"""
                            <script>
                            (function() {{
                                ...
                                // Store in cookie as well for better persistence
                                document.cookie = 'session_token={session_token}; path=/; max-age=86400';
                            }})();
                            </script>
                        """, unsafe_allow_html=True)
